...
DB: dict[str, list[dict[str, Any]]] = {
    ...
    "plans": [
        {"code": "Free", "name": "Free", "description": "免费体验", "price_monthly": 0, "status": "active"},
        {"code": "Pro", "name": "Pro", "description": "长篇小说自动生产", "price_monthly": 129, "status": "active"},
        {"code": "Team", "name": "Team", "description": "团队协作与 API", "price_monthly": 399, "status": "active"},
    ],
    "quota_balances": [
        {
            "organization_id": "org_personal",
...
def get_row(table: str, row_id: str, organization_id: str | None = None) -> dict[str, Any] | None:
    for row in DB.get(table, []):
        if row.get("id") == row_id and (organization_id is None or row.get("organization_id") == organization_id):
            return deepcopy(row)
    return None
...
def update_row(table: str, row_id: str, values: dict[str, Any]) -> dict[str, Any] | None:
    for row in DB.get(table, []):
