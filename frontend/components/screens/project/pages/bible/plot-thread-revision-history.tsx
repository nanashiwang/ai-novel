...
export type PlotThreadRevisionHistoryProps = {
  projectId: string;
  threadId: string;
};

export function PlotThreadRevisionHistory({
  projectId,
  threadId,
}: PlotThreadRevisionHistoryProps) {
  const queryClient = useQueryClient();
  const key = useScopedKey("project", projectId, "plot-thread-revisions", threadId);
  const plotThreadsKey = useScopedKey("project", projectId, "plot-threads");
  const pendingKey = useScopedKey("project", projectId, "plot-thread-pending");

  const { data: revisions = [], isPending } = useQuery({
    queryKey: key,
    queryFn: () => plotThreadRevisionsApi.list(projectId, threadId),
  });
  ...
  const apply = useMutation({
    mutationFn: (revisionId: string) =>
      plotThreadRevisionsApi.apply(projectId, threadId, revisionId),
    onSuccess: () => {
      toast.success("已应用");
      invalidate();
    },
  ...
  const reject = useMutation({
    mutationFn: (revisionId: string) =>
      plotThreadRevisionsApi.reject(projectId, threadId, revisionId),
    onSuccess: () => {
      toast.success("已拒绝");
      invalidate();
    },
  ...
  const rollback = useMutation({
    mutationFn: (revisionId: string) =>
      plotThreadRevisionsApi.rollback(projectId, threadId, revisionId),
    onSuccess: () => {
      toast.success("已回滚");
      invalidate();
    },
  ...
  return (
    <ul className="space-y-3">
      {revisions.map((rev: PlotThreadRevision) => (
        <li
          key={rev.id}
